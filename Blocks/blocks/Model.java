...
class Model {
    ...
    private int scoreClearedLines(int nrows, int ncols) {
        ...
        _streakLength += 1;
        int sumWithoutRowOverlap = nrows * _height + ncols * _width - nrows;
        int sumWithoutColOverlap = nrows * _height + ncols * _width - ncols;
        int sumOfBlooks = nrows * _height + ncols * _width;
        if (nrows >= 1 && ncols >= 1) {
            if (nrows == ncols) {
                return  sumWithoutRowOverlap + _streakLength * (sumOfBlooks);
            } else if (nrows > ncols) {
                return  sumWithoutRowOverlap + _streakLength * (sumOfBlooks);
            } else {
                return  sumWithoutRowOverlap + _streakLength * (sumOfBlooks);
        ...
        return sumOfBlooks + _streakLength * (sumOfBlooks);
